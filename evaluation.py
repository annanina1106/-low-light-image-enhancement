...
#     執行定量評估的主函數
#     """
#     # 檢查 Ground Truth 是否存在
#     gt_path = 'Streetlight in the fog.jpg'
#     if not os.path.exists(gt_path):
#         messagebox.showwarning("警告", "找不到 normal_light.jpg，無法進行評估。\n請確保檔案在同一目錄下。")
#         return

#     gt_img = cv2.imread(gt_path)

#     # 1. 模擬低光源 (Gamma=3.0，符合簡報設定)
#     input_gamma = 3.0
#     lut = np.empty((1, 256), np.uint8)
#     for i in range(256):
#         lut[0, i] = np.clip(pow(i / 255.0, input_gamma) * 255.0, 0, 255)
#     low_light_input = cv2.LUT(gt_img, lut)

#     # ================= 我們的致勝算法 (Ours) =================
    
#     # A. Modality 1: Illumination 
#     # [關鍵參數] gamma_value=2.2 用於整體提亮，clip_limit=2.0 避免雜訊
#     h, s, v, v_en = illumination.process_illumination(low_light_input, clip_limit=2.0, gamma_value=2.2)
    
#     # B. Modality 2: Structure 
#     # [關鍵參數] sigma=50 保留適度邊緣
#     struct = structure.process_structure(v, sigma=50)
    
#     # C. Fusion Strategy 
#     # [關鍵參數] 權重 0.3，符合消融實驗描述
...
#     # 自動存檔 (供簡報與 Demo 使用)
#     cv2.imwrite("eval_ours.jpg", our_result)
#     cv2.imwrite("eval_he.jpg", he_result)
#     cv2.imwrite("eval_gt.jpg", gt_img)
#     cv2.imwrite("eval_input.jpg", low_light_input)
#     print("Evaluation images saved: eval_ours.jpg, eval_he.jpg, etc.")
...
#     # gt_path = os.path.join(current_dir, 'normal_light.jpg')
#     gt_path = os.path.join(current_dir, 'Streetlight in the fog.jpg')
...
#     # A. Modality 1: Illumination (傳入 None 觸發 Region-Aware Fusion)
#     h, s, v, v_en = illumination.process_illumination(low_light_input, clip_limit=None)
    
#     # B. Modality 2: Structure (傳入 None 觸發 Adaptive Denoise)
#     struct = structure.process_structure(v, sigma=None)
    
#     # C. Fusion Strategy (傳入 None 觸發 Adaptive Weight + De-hazing)
#     our_result = fusion.fuse_images(h, s, v_en, struct, structure_weight=None)
...
# ✅ 修改：接收 image_path 參數
def run_evaluation_mode(image_path):
    """
    執行定量評估的主函數 (修復中文路徑與相對路徑問題)
    image_path: 從 UI 傳入的 Ground Truth 圖片路徑
    """
    
    # 1. 檢查路徑是否為空或不存在
    if not image_path or not os.path.exists(image_path):
        messagebox.showwarning("警告", f"找不到檔案或路徑為空：\n{image_path}")
        return

    # 2. [關鍵修正] 使用支援中文路徑的讀取方式
    gt_img = cv_imread(image_path)

    if gt_img is None:
        messagebox.showerror("嚴重錯誤", f"檔案存在但無法讀取 (可能是格式損壞):\n{image_path}")
        return

    print(f"成功讀取 Ground Truth 圖片: {image_path}, 大小: {gt_img.shape}")
    ...
    # A. Modality 1: Illumination 
    # 啟動自適應模式 (clip_limit=None) 或使用最佳化參數 (gamma_value=2.2)
    h, s, v, v_en = illumination.process_illumination(low_light_input, clip_limit=2.0, gamma_value=2.2)
...
if __name__ == "__main__":
    # 本地測試用，若直接執行此檔案，請手動指定一張圖
    # run_evaluation_mode("C:/path/to/test/image.jpg")
    pass