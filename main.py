...
# import structure
# import fusion
# import evaluation  # ✅ 新增：匯入 evaluation 模組
...
#         # 這裡的 illumination 也可以選擇是否要加入 gamma_value 供 GUI 調整
#         # 目前保持簡單，僅使用 clip_limit
...
#     cv2.createTrackbar("Detail", window_name, 30, 100, lambda x: process_pipeline()) # 預設調高一點到 0.3
...
#     print("OpenCV GUI Running... (點視窗叉叉❌即可退出，或按 s 存檔)")
#     while True:
...
#                 cv2.imwrite("gui_output.jpg", final_res)
...
#     # 使用推薦參數
#     h, s, v_orig, v_enh = illumination.process_illumination(img, clip_limit=3.0, gamma_value=2.0)
#     struct_map = structure.process_structure(v_orig, sigma=75)
#     result = fusion.fuse_images(h, s, v_enh, struct_map, structure_weight=0.3)

#     utils.show_results(img, v_enh, struct_map, result)
#     cv2.imwrite('output_enhanced.jpg', result)
#     messagebox.showinfo("完成", "處理完成！結果已儲存至 'output_enhanced.jpg'")
...
#         struct_map = structure.process_structure(v_orig, sigma=50) # sigma 稍微小一點比較快
...
#     # YOLO 預處理增強
#     h, s, v_orig, v_enh = illumination.process_illumination(img, clip_limit=3.0, gamma_value=2.0)
#     struct_map = structure.process_structure(v_orig)
#     enhanced_img = fusion.fuse_images(h, s, v_enh, struct_map, structure_weight=0.3)
...
#     plt.subplot(1, 2, 1)
#     plt.title(f"Original (Boxes: {len(results_orig[0].boxes)})")
#     plt.imshow(cv2.cvtColor(results_orig[0].plot(), cv2.COLOR_BGR2RGB))
#     plt.subplot(1, 2, 2)
#     plt.title(f"Ours Enhanced (Boxes: {len(results_enh[0].boxes)})")
#     plt.imshow(cv2.cvtColor(results_enh[0].plot(), cv2.COLOR_BGR2RGB))
#     plt.show()
...
#         h, s, v_orig, v_enh = illumination.process_illumination(frame, clip_limit=2.5, gamma_value=1.5)
...
#         self.path_var = tk.StringVar(value="1_2.png")
...
#         self.create_btn("2. Single Image Enhancement", self.trigger_single)
...
#         f = filedialog.askopenfilename(initialdir=os.getcwd(), title="Select File")
...
#         self.status_label.config(text="Processing Single Image...")
...
#         self.root.update()
#         evaluation.run_evaluation_mode() # 呼叫 evaluation.py 中的函數
...
def run_adaptive_experiment(image_path):
    ...
    cv2.imwrite("exp_baseline_he.jpg", res_fixed)
    cv2.imwrite("exp_ours_adaptive.jpg", res_auto)
...
class AppLauncher:
    ...
    def trigger_eval(self):
        self.status_label.config(text="Running Evaluation...")
        self.root.update(); target_path = self.get_path(); evaluation.run_evaluation_mode(target_path); self.status_label.config(text="Ready")
